    ...
    return {
        ...
        resolveShippingRates: function (ratesData) {
            var selectedShippingRate = checkoutData.getSelectedShippingRate(),
                availableRate = false;
console.log( ratesData.length + ' ************* ' + ratesData );
            ...
            ratesData.sort( predicateBy("amount") )
            console.log( ratesData.reverse() ); //;
            console.log( ratesData.length ); //;
            console.log( ratesData.some(item => item.method_code === 'flatrate1'));
            console.log( ratesData.some(item => item.method_code === 'flatrate2'));
            for (var j = 0; j < 11; j++) {
                var index = ratesData.findIndex(item => item.method_code == "flatrate"+j);
                console.log( index ); //;
                if (index != -1) {
                    var key = index;
                }
                ...
                var freeindex = ratesData.findIndex(item => item.method_code == "freeshipping");
                console.log( 'freeindex' + freeindex ); //;
                var fkey = freeindex;

            //console.log( 'free key >> ' + fkey ); //;
            //ratesData = ratesData.filter(x => x.method_code === "flatrate1");
            //console.log( 'final >> ' + ratesData ); //;

            if (ratesData.length >= 1) {
                console.log( 'greaterthanonemethod' ); //;
                //set shipping rate if we have only one available shipping rate
                /*
                ...
                */
                if(fkey != "" && fkey >= 0) {
                    /*
                    var ftemporary  = ratesData[0];
                    ratesData[0]   = ratesData[key];
                    ratesData[key] = ftemporary;
                    selectShippingMethodAction(ratesData[0]);
                    */
                    selectShippingMethodAction(ratesData[fkey]);
                } else if(key !="" && key >= 0) {
                    /*
                    var temporary  = ratesData[0];
                    ratesData[0]   = ratesData[key];
                    ratesData[key] = temporary;
                    selectShippingMethodAction(ratesData[0]);
                    */
                    selectShippingMethodAction(ratesData[key]);
                } else if (!selectedShippingRate) {
                    selectShippingMethodAction(ratesData[0]);
/*
                    for (var i = 0; i < ratesData.length; i++) {
                        console.log(ratesData[i]);
                        if (i != 0) {
                            ratesData.pop(); // set length to remove elements
                        }
                    }
*/
                } else {
                ...
                console.log('last >> ' + ratesData.length);
                console.log( ratesData ); //;
                //console.log(ratesData[0]);
            }
